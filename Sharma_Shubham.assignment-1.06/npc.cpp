...
void gen_monsters(dungeon_t *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    m = new npc; // alloc m object with npc size;
...
void npc_next_pos_line_of_sight(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  dir[dim_y] = get_character_yPos(d->pc) - c->position[dim_y];
  dir[dim_x] = get_character_xPos(d->pc) - c->position[dim_x];
  if (dir[dim_y]) {
...
static void npc_next_pos_00(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c, d->pc)) {
    c->npc->pc_last_known_position[dim_y] = get_character_yPos(d->pc);
    c->npc->pc_last_known_position[dim_x] = get_character_xPos(d->pc);
