...
void combat_character(dungeon_t *d, character *atk, character *def)
{
  pair_t next;
  if(atk!= d->PC && def!=d->PC)
  {
	npc_next_pos(d, (npc *) atk, next);
  }
  else
  {
...
void do_combat(dungeon_t *d, character *atk, character *def)
{
  ...
  if (pc_is_alive(d)&& atk==d->PC) {
    ...
    if (!character_is_alive(def)) {
      if (d->character_map[character_get_y(def)][character_get_x(def)] == def) {
    ...
    combat_character(d,atk,def);
    heap_insert(&d->events, update_event(d, e, 1000 / character_get_speed(def)));
  }
...
if (pc_is_alive(d)&& def==d->PC) {
  ...
  //  heap_insert(&d->events, e);
    ...
    if (!character_is_alive(atk)) {
      if (d->character_map[character_get_y(atk)][character_get_x(def)] == atk) {
        d->character_map[character_get_y(atk)][character_get_x(atk)] = def;
      }
      if (atk != d->PC) {
        event_delete(e);
      }
   ...
   combat_character(d,atk,def);
    heap_insert(&d->events, update_event(d, e, 1000 / character_get_speed(atk)));
  }
