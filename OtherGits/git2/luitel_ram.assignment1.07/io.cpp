...
from[dim_y] = max(1, character_get_y(d->PC) - PC_VISUAL_RANGE);
  from[dim_x] = max(1, character_get_x(d->PC) - PC_VISUAL_RANGE);
  to[dim_y] = min(DUNGEON_Y - 1, character_get_y(d->PC) + PC_VISUAL_RANGE);
  to[dim_x] = min(DUNGEON_X - 1, character_get_x(d->PC) + PC_VISUAL_RANGE);

  c =(character**)malloc(d->num_monsters * sizeof (*c));

  /* Get a linear list of monsters */
  for (count = 0, y = from[dim_y]; y <= (size_t)to[dim_y]; y++) {
    for (x = from[dim_x]; x <= (size_t)to[dim_x]; x++) {
