...
void gen_monsters(dungeon_t *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
   ...
   // m->pc = NULL;
    ...
    m->pc_seen = 0;
    m->last_position[dim_y] = 0;
    m->last_position[dim_x] = 0;
...
void npc_next_pos_line_of_sight(dungeon_t *d, character *c, pair_t next)
{
  ...
  dir[dim_y] = get_y(d->thepc) - c->position[dim_y];
  dir[dim_x] = get_x(d->thepc) - c->position[dim_x];
  if (dir[dim_y]) {
...
static void npc_next_pos_00(dungeon_t *d, npc *c, pair_t next)
{
  /* not smart; not telepathic; not tunneling; not erratic */
    if (can_see(d, character_get_pos(c), character_get_pos(d->thepc), 0)) {
    c->last_position[dim_y] = get_y(d->thepc);
    c->last_position[dim_x] = get_x(d->thepc);
...
static void npc_next_pos_01(dungeon_t *d, npc *c, pair_t next)
{
 ...
 if (can_see(d, character_get_pos(c), character_get_pos(d->thepc), 0)) {
  ...
  if ((next[dim_x] == c->last_position[dim_x]) &&
      (next[dim_y] == c->last_position[dim_y])) {
    c->pc_seen = 0;
  }
}
