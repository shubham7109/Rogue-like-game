...
{
  d->io_offset[dim_x] = ((d->character_map[get_x(d->thepc)] - 10) / 20) * 20;
...
uint32_t io_teleport_pc(dungeon_t *d)
{
  ...
  d->character_map[get_y(d->thepc)][get_x(d->thepc)] = NULL;
  d->character_map[dest[dim_y]][dest[dim_x]] = d->thepc;

  set_y(d->thepc, dest[dim_y]);
  set_x(d->thepc, dest[dim_x]);
...
static int io_list_monsters_display(dungeon_t *d,
                                     character **c,
                                     uint32_t count)
{
  uint32_t i;
  ...
  for (i = 0; i < count; i++) {
    snprintf(s[i], 40, "%16s%c: %2d %s by %2d %s",
             ...
             abs(get_y(c[i]) - get_y(d->thepc)),
             ((get_y(c[i]) - get_y(d->thepc)) <= 0 ?
              "North" : "South"),
             abs(get_x(c[i]) - get_x(d->thepc)),
             ((get_x(c[i]) - get_x(d->thepc)) <= 0 ?
              "East" : "West"));
    if (count <= 13) {
...
static int compare_monster_distance(const void *v1, const void *v2)
{
  ...
  const character *const *c2 = v2;
	  ...
	  return (dungeon->pc_distance[get_y(*c1)][get_x(*c2)] -
          dungeon->pc_distance[get_y(*c1)][get_x(*c2)]);
}
...
static void io_list_monsters(dungeon_t *d)
{
 ...
 from[dim_y] = max(1, get_y(d->thepc) - PC_VISUAL_RANGE);
  from[dim_x] = max(1, get_x(d->thepc) - PC_VISUAL_RANGE);
  to[dim_y] = min(DUNGEON_Y - 1, get_y(d->thepc) + PC_VISUAL_RANGE);
  to[dim_x] = min(DUNGEON_X - 1, get_x(d->thepc) + PC_VISUAL_RANGE);
  ...
  for (count = 0, y = from[dim_y]; y <= to[dim_y]; y++) {
    for (x = from[dim_x]; x <= to[dim_x]; x++) {
      if (d->character_map[y][x] && d->character_map[y][x] != d->thepc) {
