...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
	...
	}else if (d->objmap[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x] &&
                 (d->objmap[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->seen ||
                  can_see(d, character_get_pos(d->PC), pos, 1))) {

        attron(COLOR_PAIR(d->objmap[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->color));

        mvaddch(pos[dim_y] + 1, pos[dim_x],
                d->objmap[dim_y][d->io_offset[dim_x] + x]->symbol);

        attroff(COLOR_PAIR(d->objmap[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->color));

		/*
		 }else if(d->objmap[pos[d->io_offset[dim_y] + y]]
                          [d->io_offset[dim_x] + x]){	
				attron(COLOR_PAIR(d->objmap[d->io_offset[dim_y] + y]
                          [d->io_offset[dim_x] + x]->color));		  
						  
			mvaddch(y + 1, x,character_get_symbol(d->objmap[d->io_offset[dim_y] + y]
                                                     [d->io_offset[dim_x] + x]->symbol)); 		  
			//character_get_symbol(
			attron(COLOR_PAIR(d->objmap[d->io_offset[dim_y] + y]
                          [d->io_offset[dim_x] + x]->color));		  
